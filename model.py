...
##    h_pool2 = self._max_pool_2x2(h_conv2)
    ...
    #W_fc1 = self._weight_variable([28*28, 1024])
    W_fc1 = tf.Variable(tf.truncated_normal([28*28,1024], stddev=0.1), name='W')
    #b_fc1 = self._bias_variable([1024])

    #h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
    #h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
    #h_fc1 = tf.nn.relu(tf.matmul(self.x_input, W_fc1) + b_fc1)
    h_fc1 = tf.nn.relu(tf.matmul(self.x_input, W_fc1))

    # output layer
    #W_fc2 = self._weight_variable([1024,10])
    W_fc2 = tf.Variable(tf.truncated_normal([1024, 10], stddev=0.1), name='V')
    #b_fc2 = self._bias_variable([10])

    #self.pre_softmax = tf.matmul(h_fc1, W_fc2) + b_fc2
    self.pre_softmax = tf.matmul(h_fc1, W_fc2)
